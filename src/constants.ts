//Global Params
export const CHEATING = true
export const SPEED = 2

//Player Movement Params
export const JUMP_POWER = 0.01
export const JUMP_X = 0.2
export const JUMP_Y = 0.4
export const MOVE_SPEED = 0.0004


//Player Style Params
export const PLAYER_WIDTH = 30
export const PLAYER_HEIGHT = 30
export const PLAYER_COLOR = 0xFF0000
export const PLAYER_BORDER = 0xFFFFFF
export const PLAYER_BORDER_THICKNESS = 1

//Player Physics Params
export const PLAYER_DENSITY = 0.010
export const PLAYER_RESTITUTION = 0.1
export const PLAYER_FRICTION = 0.1
export const PLAYER_FRICTION_AIR = 0
export const PLAYER_INERTIA = Infinity
export const PLAYER_FRICTION_STATIC = 0.5


//Matter Engine Params
export const GRAVITY_X = 0
export const GRAVITY_Y = 0.1


//Outside Walls Params
export const WALL_RESTITUTION = .6
export const WALL_FRICTION = 0.1
export const WALL_WIDTH = 1

//Ground params
export const GROUND_WIDTH = 2
export const GROUND_RESTITUION = 0
export const GROUND_FRICTION = 1